...
class AI:
    ...
    def select_move(self, board):
        ...
        #ランダムに指し手を選ぶ
        r = random.randient(0, len(move_list) - 1)
        return move_list[r]
...
def draw_board():
    ...
    for y in range(8):
        for x in range(8):
            disk = game.board.board[y][x]
            if disk != SPACE:
                color = "black"
            else:
                color = "white"
            #石の描画
            canvas_board.create_oval(x*CELL_PX_SIZE+4,y*CELL_PX_SIZE+4, (x+1)*CELL_PX_SIZE, (y+1)*CELL_PX_SIZE, fill = color)
    #枠を描画
    for x in range(8):
...
def disp_mess():
    ...
    if game.game_mode == 0:
        ...
        if game.board.turn == BLACK:
            ...
            mess += u"白が打てよ"
        #白黒の石数をタプルで習得
        (black_disks, white_disks) = game.board.get_disks()
        mess += "黒："　+ str(black_disks) + "白：" +str(white_disks)

    elif game.game_mode == 2:
        #白黒の石数をタプルで習得
        (black_disks, white_disks) = game.board.get_disks()
        mess = u"対局終了" + str(game.board.move_num-1) + u"手" + "黒：" + str(black_disks)+" 白：" + str(white_disks)
        if black_disks == white_disks:
            mess += u"引き分け"
        elif black_disks > white_disks:
            mess += u"黒の勝ち"
        else:
            mess += u"白の勝ち"
